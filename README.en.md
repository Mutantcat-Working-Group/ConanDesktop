...
### 2. Running and Deployment
1. Prepare the local environment
   ```
   Node.js 22.12+, pnpm, Rust stable, and the Tauri 2 system dependencies for the platform
   Conan 2 (currently tested with 2.16.1); it is not bundled with the app
   CMake (optional) for automatic location detection on the project page
   ```
2. Run in development mode
   ...
   pnpm tauri dev
   ```
   Run `pnpm dev` for frontend preview only; the default address is `http://localhost:1420`. A browser can preview the UI but cannot call local Conan.
3. Build locally
   ```sh
   pnpm tauri build
   ```
...
4. Publish a release
   ...
   ```
   Pushing a `v*` tag makes GitHub Actions build a Windows NSIS installer, both macOS DMG architectures, and a Linux AppImage. After all four installers succeed, it generates `SHA256SUMS.txt`, creates a draft Release, uploads the assets, and publishes it. See the [release guide](docs/releasing.md).

### 3. Usage Guide
1. Configure the application
   ```
   Set the Conan executable and the default project directory in "App Settings".
   ```
...
3. Browse packages
   ```
   Select a remote repository, enter a reference or wildcard such as engine/*@team/stable, then inspect versions, revisions, and binary details.
   ```
4. Configure profiles
   ```
   Create, auto-detect, or edit a profile in raw text. After validation, switch it to active, staged, or inactive.
   Only the active state can be bound to a project.
   ```
   Profile state is application metadata and never rewrites Conan's global default configuration. A profile referenced by a project cannot be deleted, renamed, or deactivated until that binding is removed.
5. Configure projects
   ```
   Create a project, select a local directory, and automatically scan Conan/CMake files to locate CMake.
   Bind an active profile, then add environment variables and arguments as needed.
   ```
   Scanning never runs configure/install/build and does not write the project's `CMakePresets.json`.

### 4. Data and Security Notes
